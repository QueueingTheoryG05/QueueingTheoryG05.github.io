(() => {
  ...
  const hashMatch =
    window.location.hash.match(/^#slide-(\d+)$/);

  let current = hashMatch
    ? clamp(
  ...
  function fitSlideContent(slide) {
    ...
    /*
      Typography remains identical between slides.
      The outer presentation stage handles uniform scaling.
    */
  ...
  function updateInterface(
    ...
    const chapter =
      slide.dataset.chapter || 'Presentation';
    ...
    document.title =
      `${pad(current + 1)} · ${title} | Queueing Theory`;
  ...
  */
  ...
  const fullscreenHostDocument = (() => {
    try {
      return fullscreenHostWindow.document;
    } catch (_) {
      return document;
    }
  })();
  ...
  const fullscreenTarget = (() => {
    try {
      /*
        Fullscreen the outer index.html document,
        not the iframe itself.

        This keeps the iframe CSS zoom active
        and prevents the text from shrinking.
      */
      return (
        fullscreenHostDocument
          .documentElement
      );
    } catch (_) {
  ...
  function createImageLightbox() {
    ...
    document
      ...
      .forEach((image) => {
        ...
        image.setAttribute(
          'aria-label',
          `${image.alt || 'Figure'} — open full image`
        );
  ...
  slideMap?.addEventListener(
    'mousedown',
    (event) => {
      if (event.target === slideMap) {
        closeOverview();
      }
    }
  );
